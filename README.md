...
应用为 Windows x64 self-contained 便携包，不要求预装 .NET、Node.js、npm 或 npx，不要求管理员权限。当前 `1.3.1` 未进行代码签名，因此 Windows 可能显示未知发布者警告；只应使用哈希与发布记录匹配的包。

首次启动会使用 `%LOCALAPPDATA%\CodexThemeStudio\Data` 作为明确的默认数据目录，不会自动启用持久化。应用就绪后会访问公开 GitHub Release API 检查稳定更新；不会上传主题、图片或诊断材料。程序目录可以移动，但用户数据和已安装的持久化 Agent 不存放在程序目录内。
...
- 在“设置 → 关于”主动检查更新，或点击各页面标题旁的绿色更新图标直接在当前页面强制检查；检查失败后图标会保留以便重试。发现稳定版后可查看 Release Notes，并在带阶段文案、进度条和百分比的弹窗中下载、三重校验、重启安装。安装失败会自动恢复旧目录，未知或被修改的旧文件会单独保留。
...
- 当前经过真实验证的 Codex 版本为 `26.715.4045.0`、`26.715.10079.0`、`26.721.3404.0`、`26.727.6591.0`、`26.730.8199.0`、`26.803.5235.0` 和 `26.803.10989.0`。这些记录是兼容证据，不是运行白名单；未知版本在完整能力探测通过后可以临时使用。
...
- 当前发布形态是解压即用的便携目录，不是单文件 EXE。`1.3.0` 起支持用户主动触发的自更新，不执行后台静默安装；`1.2.2` 及更早版本需手动安装 `1.3.0` 后才能使用该能力。
- 当前 `1.3.1` 未签名；Windows 可能显示未知发布者警告。
- `1.3.1` 未在无预装 .NET/Node 的干净 Windows VM 或有效 Defender 环境完成发布验收，真实 updater 进程终止与断电边界也未覆盖；这些风险不应被理解为已通过测试，首次使用或更新前应保留可恢复副本。
