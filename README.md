...
应用为 Windows x64 self-contained 便携包，不要求预装 .NET、Node.js、npm 或 npx，不要求管理员权限。当前 `1.3.0` 未进行代码签名，因此 Windows 可能显示未知发布者警告；只应使用哈希与发布记录匹配的包。
...
- 当前 `1.3.0` 未签名；Windows 可能显示未知发布者警告。
- `1.3.0` 未在无预装 .NET/Node 的干净 Windows VM 或有效 Defender 环境完成发布验收，真实 updater 进程终止与断电边界也未覆盖；这些风险不应被理解为已通过测试，首次使用或更新前应保留可恢复副本。
