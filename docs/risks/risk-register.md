...
| R-07 | 能力探测可能无法覆盖未来 Codex 或第三方构建的全部行为差异 | 当前真实证据覆盖 Store Codex `26.715.4045.0`、`26.715.10079.0`、`26.721.3404.0` 与 `26.727.6591.0` x64；最新版本 Electron 为 `150.0.7871.182`，完整能力探测与应用/清理/重应用闭环通过。Injector 已适配端口监听早于 HTTP 就绪及关闭后重开延迟；能力缺失、身份变化、错误端口所有者和清理残留仍 fail-closed。非 Store 真实实例验证为 `To be confirmed` | Open |
