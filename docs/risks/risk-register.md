...
- 更新日期：2026-08-12
- 适用基线：Codex Theme Studio 1.3.1 本地维护发布
...
| R-03 | 本机没有有效病毒扫描证据 | Defender 服务、实时保护和签名库当前不可用；`v1.3.1` 必须在 README、验收记录和 Release Notes 披露，公开前另行取得针对本版本的风险接受，后续仍应在有效环境复扫 | Open |
| R-04 | 未在无开发 Runtime 的干净 Windows 用户或 VM 验收 | self-contained 文件、随包 Node、中文/空格路径启动及当前主机可运行性已有证据，但不能替代无预装 .NET/Node 的干净环境；`v1.3.1` 公开前必须披露并另行取得针对本版本的风险接受 | Open |
...
| R-07 | 能力探测可能无法覆盖未来 Codex 或第三方构建的全部行为差异 | 当前真实证据覆盖 Store Codex `26.715.4045.0`、`26.715.10079.0`、`26.721.3404.0`、`26.727.6591.0`、`26.730.8199.0`、`26.803.5235.0` 与 `26.803.10989.0` x64；新增两版已完成应用生命周期和 Codex 重启恢复验收，Electron 精确版本为 `To be confirmed`。能力缺失、身份变化、错误端口所有者和清理残留仍 fail-closed。非 Store 真实实例验证为 `To be confirmed` | Open |
...
| R-16 | 旧本机交接快照或历史归档可能被误认为当前实施状态 | `Directory.Build.props` 固定维护基线 `1.3.1`；根目录旧交接快照已可恢复移出并由 `.gitignore` 阻止再次误提交，历史验收文档只保留证据边界 | Mitigated |
...
| R-22 | 自更新在文件锁、断电或新版启动失败时留下不可运行目录 | 外置 Node runner 只接收随机 token；请求通过 Schema v1 JSON 传递。应用目录在同盘以 rename 切换，旧目录保留到新版完成 120 秒健康检查；自动化已覆盖文件锁、启动超时、回滚及回滚失败。`v1.3.1` 尚未覆盖断电及真实杀进程边界，风险必须公开披露并另行接受 | Open |
