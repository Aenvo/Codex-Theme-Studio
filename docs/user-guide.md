...
1. 先启动 Codex。默认自动检测当前用户的 Microsoft Store Codex；需要使用其他安装来源时，在“设置 → Codex 连接”选择目标 EXE 并确认来源警告。
2. 在主题库选中主题，点击“临时显示”。
3. 选中另一个主题并再次临时显示即可切换。

临时显示只影响当前 Codex 实例，不创建启动项，不启动持久化 Agent。Codex 完全退出后效果消失。

未知版本或新的 EXE 指纹在能力探测通过后即可临时显示。第一次临时显示会自动执行“应用 → 清理 → 重新应用”验证，成功后主题保持可见，并开放“设为持久主题”。如果目标 EXE 内容发生变化，需要对新指纹重新确认来源并再次完成该闭环。

## 启用持久化

当前 EXE 指纹必须是已验证版本，或已成功完成首次临时闭环；否则按钮保持禁用并显示操作指引。
