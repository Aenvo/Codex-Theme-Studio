...
编辑器生成的临时受管背景使用独立的生命周期规则。当前编辑会话会记录新生成
资源；取消新草稿、成功保存替换背景或新副本保存失败时，只对已证明不再被最终
`theme.json` 的 `art.file` 引用的精确资源执行永久删除，不经过 Windows 回收站。
存储层在删除前再次验证主题 UUID、可信 DataRoot、普通文件/目录、重解析点、
当前主题引用和空目录条件；验证失败时不删除。共享缓存、索引主题、应用回收站
主题、未知孤立目录和完整但未索引主题不属于该规则。
