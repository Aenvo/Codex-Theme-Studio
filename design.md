...
- **Confirmed**：应用固定使用以 `#111111` 为主背景的中性黑桌面视觉，并以蓝色 `#2563EB` 表达主要动作、选择和键盘焦点；不跟随 Windows 明暗模式。
...
UI 变更至少检查默认窗口与最小窗口下的空态、单主题、100 主题、编辑器、设置、加载、错误、禁用、Hover、Focus、Popup 和滚动。主题资料库还必须检查卡片/列表默认态与双向切换、分段器 Tooltip、共享选择状态，以及两种布局的虚拟化。源码与 ViewModel 测试不能替代真实渲染截图和人工目视检查。
